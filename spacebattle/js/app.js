      ...
      if (alien.name === alienShip[0].name) {
        ...
        if (Math.floor(Math.random() * Math.floor(9)) / 10 <= this.accuracy) {
          ...
          if (myMissles[0] == "M") {
            ...
            console.log(
              alien.name + " got hit with a missle, their health is down to"
            );
            console.log((alien.hull += -10));
            myMissles[myMissles.length] = "o";
            myMissles.splice(0, 1);
            victory(alien); ///// check if you destroted the ship or not and they or the next one returns fire
          } else {
            console.log(
          ...
          victory(alien); //// you missed but that was your turn they get to shoot at you now
        }
      } else {
        console.log(
    ...
    if (this.hull >= 1) {
      ...
      if (alien.name === alienShip[0].name) {
        if (Math.floor(Math.random() * Math.floor(9)) / 10 <= this.accuracy) {
          ...
          console.log((alien.hull += -this.weapons.lasers));
          victory(alien);
        } else {
          console.log(
...
let earthShip = new USSship(" USS Schwarzenegger ");
  ...
  fight(earthShip) {
    if (this.name === alienShip[0].name) {
      if (Math.floor(Math.random() * Math.floor(9)) / 10 <= this.accuracy) {
        ...
        console.log((earthShip.hull -= this.firepower));
        defeat(earthShip); /// but did you die? also returns it to your move
      } else {
        ...
        defeat(earthShip); /// returns to your move no damage dealt - live to fight another day
      }
    } else {
  ...
  attack(earthShip) {
    if (this.name === alienShip[0].name) {
      if (Math.floor(Math.random() * Math.floor(9)) / 10 <= this.accuracy) {
        ...
        console.log((earthShip.hull += -7)); /// had it set to this . mizzle but somehting was not working with it or  the random number so I hard coded it... sorry
        defeat(earthShip); /// like above -_- but did you die?
      } else {
        ...
        defeat(earthShip); /// no damage awesome but return to your move with this
      }
    } else {
      console.log("dead aliens can't shoot"); // not sure how they would but just in case if simulating in console log with pre typed functions...
      //return to your move
...
//// will build last alien ship first then each subsequent pushed before it into array -
// construct in reverse order because I want ship 1 to be first target .... it's my game....

alienShip.unshift((al7 = new Alienship("AL #7")));
alienShip.unshift((al6 = new Alienship("AL #6")));
alienShip.unshift((al5 = new Alienship("AL #5")));
alienShip.unshift((al4 = new Alienship("AL #4")));
alienShip.unshift((al3 = new Alienship("AL #3")));
alienShip.unshift((al2 = new Alienship("AL #2")));
alienShip.unshift((al1 = new Alienship("AL #1")));

/// they are generated with random attributes - a loop could auto create them but not sure how to have it give them each different names unless I tie it to an array of  names..... lots of other stuff to fix before that...  will have to use name of alien ship to attack them and name of alien to attack us back..
...
const battleWinCheck = (alien) => {
  if (alien.hull <= 0) {
    console.log("You blew up " + alien.name + " Nice job!");
    victory();
  } else {
...
///  else ... if not over have next alien ship in [ alien ships ] attack you
...
const didIWin = () => {
  if (alienShip[0] === "dead") {
    ...
    if (alienShip[3] === "dead") {
      alienShip[0].attack(earthShip);
    } else {
      alienShip[0].fight(earthShip); /// al#5 al#6 and al#7 will use more powerful mizzles
...
///// lets make it so a alien ship ship can be destroyed
...
const defeat = (earthShip) => {
  if (earthShip.hull <= 0) {
    ...
    alert(
      ` Oh No your ship blew up ! The ${earthShip.name} is gone! Close this annoying pop-up and refresh the page to try again Captian! Good thing it was just a simulation..... don't forget your engineers can search Reddit for a way to restore your hull power`
    );
  } else {
    console.log(
      "%c .. Your turn now captian enter your move below .. ",
